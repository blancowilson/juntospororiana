...
echo ""
echo "E.3 Instalar backup automatico (local + offsite):"
echo "   sudo mkdir -p /var/backups/juntospororiana"
echo "   sudo chown ${APP_USER}:${APP_USER} /var/backups/juntospororiana"
echo "   # Para backup externo via rclone (recomendado: B2, S3 o Google Drive):"
echo "   sudo apt install -y rclone"
echo "   sudo -u ${APP_USER} rclone config   # crea el remoto 'mi-remoto'"
echo "   # Editar el .env del proyecto y agregar (SIN comillas):"
echo "   sudo -u ${APP_USER} bash -c 'echo \"BACKUP_REMOTE_TYPE=rclone\" >> ${APP_DIR}/.env'"
echo "   sudo -u ${APP_USER} bash -c 'echo \"BACKUP_REMOTE_PATH=mi-remoto:bucket/juntospororiana\" >> ${APP_DIR}/.env'"
echo "   # Activar el timer (corre todos los dias 03:30 UTC):"
echo "   sudo cp ${APP_DIR}/vps_deploy/juntospororiana-backup.service /etc/systemd/system/"
echo "   sudo cp ${APP_DIR}/vps_deploy/juntospororiana-backup.timer /etc/systemd/system/"
echo "   sudo systemctl daemon-reload"
echo "   sudo systemctl enable --now juntospororiana-backup.timer"
echo "   # Verificar:"
echo "   systemctl list-timers juntospororiana-backup.timer"
echo "   # Probar manualmente:"
echo "   sudo -u ${APP_USER} bash ${APP_DIR}/scripts/backup_db.sh"
echo ""
